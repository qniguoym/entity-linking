...
def _get_clean_wp_text(article_text):
    ...
    # pre_l = len(clean_text)
    # sim = clean_text
    clean_text = file_regex.sub("", clean_text)
    # if len(clean_text) < pre_l:
...
def read_training_indices(lang, entity_file_path_):
    ...
    dev_indices = []
    for l in lang:
        entity_file_path = entity_file_path_%l
        with open(entity_file_path, "r", encoding="utf8") as file:
            for i, line in enumerate(file):
                example = json.loads(line)
                article_id = l+'_'+example["article_id"]
                clean_text = example["clean_text"]

                if is_valid_article(clean_text):
                    if is_dev(article_id): ###以'3'结尾的
...
def read_training_data(lang, entity_file_path_):
    ...
    for l in lang:
        ...
        with open(entity_file_path, "r", encoding="utf8") as file:
            for i, line in enumerate(file):
                ...
                article_id = l+'_'+example["article_id"]
                clean_text = example["clean_text"]
                data[article_id] = {'clean_text':clean_text,'entities':example['entities']}

    return data


def read_el_docs_golds(nlp, entity_file_path, dev, line_ids, kb):
...
def read_golds(data, line_ids, kb):

    for article_id in line_ids:
        example = data[article_id]
        clean_text = example["clean_text"]
        entities = example["entities"]
        doc = clean_text
        gold_entities = _get_gold_parse(doc, entities, kb=kb)
        if gold and len(gold_entities) > 0:
            yield doc, gold_entities


def _get_gold_parse(doc, entities, kb):
    gold_entities = {}
    ...
    for entity in entities:
        ...
        should_add_ent = entity_id in candidate_ids
