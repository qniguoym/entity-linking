...
class DenseHNSWFlatIndexer(DenseIndexer):
    """
     Efficient index for retrieval.
     Note: default settings are for hugh accuracy but also high RAM usage
    """
    ...
    def index_data(self, data: np.array):
        ...
        # max norm is required before putting all vectors
        # in the index to convert inner product similarity to L2
        if self.phi > 0:
            raise RuntimeError(
...
def get_candidates(entity2id, entity2description, tokenizer, max_length, debug=False):
    ## add description
    # return {
    #     "tokens": cand_tokens,
    #     "ids": input_ids,
    ...
    # }
    data = []
    for entity_id in sorted(entity2id.items(),key=lambda x:x[1]):
        entity=entity_id[0]
        id = entity_id[1]
        if entity not in entity2description:
            description = entity ##Q,..
        else:
            description = entity2description[entity]
        item = get_candidate_representation(description,tokenizer,max_length)
        item['id'] = id
        item['entity'] = entity
        data.append(item)
        if debug:
    ...
    cand_ids = torch.tensor(
        select_field(data, "ids"), dtype=torch.long,
    )
    cand_mask = torch.tensor(
        select_field(data, "mask"), dtype=torch.long,
    )
    cand_segment = torch.tensor(
        select_field(data, "segment"), dtype=torch.long,
    )

    tensor_data = TensorDataset(cand_ids, cand_mask, cand_segment)
